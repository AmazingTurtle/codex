...
class ReleaseTests(unittest.TestCase):
    ...
    def receipts(self, state):
        ...
        for name in ("build", "smoke"):
            state["checks"][name]["files"] = release.package_files(
                self.package, release.read_product_version(self.repo)
            )
        return state
    ...
    def test_prepare_explicit_downstream_revision(self):
        release.prepare(self.state_path, "rust-v1.2.3", "rust-v1.2.3", revision=1)
        state = json.loads(self.state_path.read_text())
        self.assertEqual(state["release_tag"], "v1.2.3-better-codex.1")

    def test_prepare_refuses_existing_or_older_revision(self):
    ...
    def test_existing_version_uses_downstream_revision(self):
        ...
        manifest = self.package / "codex-package.json"
        metadata = json.loads(manifest.read_text())
        metadata["version"] = "1.2.3-better-codex.1"
