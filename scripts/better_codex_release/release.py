...
def check_version(state):
    ...
    require(
        state["release_tag"] == f"v{read_product_version(ROOT)}",
        "Set the product-info VERSION suffix to match the prepared release tag before validation",
    )
    return version


def prepare(path, previous_tag, target_tag):
    ...
    save(
        ...
        {
            "repository": str(ROOT),
            "release_tag": next_release_tag(target_tag.removeprefix("rust-v")),
...
def main():
    ...
    if args.action == "prepare":
        prepare(path, args.previous_tag, args.upstream_tag)
        return
