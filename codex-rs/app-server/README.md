...
- `webSearch` — `{id, query, action?, results?}` for a web search request issued by the agent; `action` mirrors the Responses API web_search action payload (`search`, `open_page`, `find_in_page`) and may be omitted until completion. For standalone web search, `query` contains the best available display detail (including a resolved URL for page navigation when available), while `results` contains the out-of-band structured result DTOs returned by `/v1/alpha/search`; clients should ignore result types and fields they do not understand.
...
- `account/rateLimitResetCredit/consume` — consume one earned reset using a caller-provided idempotency key, optionally selecting a reset-credit ID returned by `account/rateLimits/read`. Pass `accountId` to redeem for an eligible stored ChatGPT account without switching the active account; omission preserves active-account behavior. Account-specific credit details are available through `account/rateLimits/readMany`.
