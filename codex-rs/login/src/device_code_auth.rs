...
fn device_code_prompt(verification_url: &str, code: &str) -> String {
    let version = codex_product_info::VERSION;
    let display_name = codex_product_info::DISPLAY_NAME;
    format!(
        "\nWelcome to {display_name} [v{ANSI_GRAY}{version}{ANSI_RESET}]\n{ANSI_GRAY}A downstream distribution of OpenAI's command-line coding agent{ANSI_RESET}\n\
\nFollow these steps to sign in with ChatGPT using device code authorization:\n\
\n1. Open this link in your browser and sign in to your account\n   {ANSI_BLUE}{verification_url}{ANSI_RESET}\n\
\n2. Enter this one-time code {ANSI_GRAY}(expires in 15 minutes){ANSI_RESET}\n   {ANSI_BLUE}{code}{ANSI_RESET}\n\
\n{ANSI_GRAY}Continue only if you started this login in {display_name}. If a website or another person gave you this code, cancel.{ANSI_RESET}\n",
    )
}
