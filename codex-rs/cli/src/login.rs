...
//! interactive-session layers. Direct `better-codex login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `better-codex-login.log` artifact that
...
/// Installs a small file-backed tracing layer for direct `better-codex login` flows.
///
...
/// command produce a durable `better-codex-login.log` artifact without coupling it to the TUI's broader
...
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    ...
    // Direct `better-codex login` otherwise relies on ephemeral stderr and browser output.
...
/// `better-codex login` working in environments where device-code may be disabled/feature-gated.
        ...
        .await
    {
        Ok(Some(auth)) => match auth.auth_mode() {
            ...
            AuthMode::Chatgpt | AuthMode::ChatgptAuthTokens => {
                // Keep this line stable for scripts that use `better-codex login status`
...
async fn run_logout_selection(
    ...
    if let LogoutSelection::Account(account_selector) = selection {
        if logged_out {
