...
struct Args {
    /// Path to Better Codex home. Defaults to $BETTER_CODEX_HOME or ~/.better-codex.
    #[arg(long, env = "BETTER_CODEX_HOME")]
...
mod tests {
    ...
    /// Explicit database selection must not parse an overridden Better Codex home.
    #[tokio::test]
    async fn direct_db_skips_codex_home_config() {
        let codex_home = tempfile::tempdir().expect("create Better Codex home");
