...
fn command(destination: &Path) -> Result<assert_cmd::Command> {
    let mut command = assert_cmd::Command::new(codex_utils_cargo_bin::cargo_bin(
        codex_product_info::CLI_NAME,
    )?);
    command.env(codex_product_info::HOME_ENV, destination);
    Ok(command)
}
...
async fn imports_persistent_state_credentials_and_consistent_sqlite_snapshot() -> Result<()> {
    ...
    let imported_database = sqlite
        .open_read_only_pool(&imported_database_path, /*busy_timeout*/ None)
        .await?;
