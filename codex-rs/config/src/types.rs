...
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in BETTER_CODEX_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in BETTER_CODEX_HOME.
    Auto,
...
pub enum OAuthCredentialsStoreMode {
    ...
    #[default]
    Auto,
    /// BETTER_CODEX_HOME/.credentials.json
...
/// Controls how code-mode tool calls are presented in normal TUI history.
#[derive(Serialize, Deserialize, Debug, Clone, Copy, PartialEq, Eq, Default, JsonSchema)]
#[serde(rename_all = "kebab-case")]
pub enum CodeModeToolCallDisplay {
    /// Show the complete invocation and result for each code-mode tool call.
    #[default]
    Individual,
    /// Show the compact title and meaningful output summary introduced in 0.148.
    Summary,
}
...
pub struct Tui {
    ...
    /// Controls how code-mode tool calls are rendered in normal conversation history.
    /// Defaults to `individual`.
    #[serde(default)]
    pub code_mode_tool_call_display: CodeModeToolCallDisplay,
    ...
    /// Use `/theme` in the TUI or see `$BETTER_CODEX_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against BETTER_CODEX_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
