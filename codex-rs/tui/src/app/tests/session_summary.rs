...
async fn session_summary_skips_resume_hint_until_rollout_exists() {
    ...
    assert!(session_summary(usage, Some(conversation), Some(&rollout_path)).is_none());
}
...
async fn session_summary_includes_resume_hint_for_persisted_rollout() {
    ...
    let summary = session_summary(usage, Some(conversation), Some(&rollout_path)).expect("summary");
    assert_eq!(
        summary,
        SessionSummary {
            usage_line: Some("Token usage: total=12 input=10 output=2".to_string()),
            resume_hint: Some(
