...
#[tokio::test]
async fn individual_tool_call_display_expands_searches_and_images() {
    ...
    chat.on_web_search_end(
        ...
        search_action("first query"),
    );
    chat.on_web_search_begin("search-2".to_string());
    chat.on_web_search_end(
        "search-2".to_string(),
        "second query".to_string(),
        search_action("second query"),
    );

    let rendered = active_blob(&chat);
    assert_eq!(rendered.matches("• Searched the web for").count(), 2);
    assert!(rendered.contains("first query"));
    assert!(rendered.contains("second query"));
    ...
    let rendered = active_blob(&chat);
    assert_eq!(rendered.matches("• Viewed Image\n").count(), 2);
    assert!(rendered.contains("first.png"));
    assert!(rendered.contains("second.png"));
}
