...
#[tokio::test]
async fn usage_picker_preserves_explicit_usage_commands() {
    for (args, view, selector) in [
        ("cumulative", TokenActivityView::Cumulative, None),
        ("daily other", TokenActivityView::Daily, Some("other")),
        ("weekly other", TokenActivityView::Weekly, Some("other")),
        ("other", TokenActivityView::Cumulative, Some("other")),
    ] {
        let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
        set_chatgpt_auth(&mut chat);
        ...
        assert_eq!((actual_view, actual_selector.as_deref()), (view, selector));
    }
}
